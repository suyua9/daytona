# daytona_toolbox_api_client_async.PortApi
...
async with daytona_toolbox_api_client_async.ApiClient(configuration) as api_client:
    ...
    api_instance = daytona_toolbox_api_client_async.PortApi(api_client)

    try:
        # Get active ports
        api_response = await api_instance.get_ports()
...
async with daytona_toolbox_api_client_async.ApiClient(configuration) as api_client:
    ...
    try:
        # Check if port is in use
        api_response = await api_instance.is_port_in_use(port)
